...
const StatusCard: React.FC<StatusCardProps> = memo(({ 
  ...
  const statusConfig = {
    ...
    warning: { color: 'text-orange-500', bg: 'bg-orange-500/10', border: 'border-orange-500/20', label: t('warning') },
    normal: { color: 'text-blue-500', bg: 'bg-blue-500/10', border: 'border-blue-500/20', label: t('normal') },
  ...
  return (
    <Card className={`p-5 flex flex-col justify-between glass-panel border-border-dark hover:border-brand-500/30 transition-all ${className}`} hoverEffect={true}>
